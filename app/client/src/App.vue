...
onMounted(async () => {
  try {
    const { user } = await graphqlWithAuth<{ user: Author }>(GITHUB_AUTHOR_QUERY, {
      login: "mktcode",
    });
...
<template>
  <main>
    ...
    <div>
      ...
      <p class="text-xl font-bold">Followers fork count: {{ followersForkCount }}</p>
      <p class="text-xl font-bold">Stargazer count: {{ stargazerCount }}</p>
      <p class="text-xl font-bold">Followers stargazer count: {{ followersStargazerCount }}</p>
      <p class="text-xl font-bold">Followers follower count: {{ followersFollowerCount }}</p>
    </div>
  </main>
</template>
